...
func (t *TraceTask) Run(_ context.Context, state *State) error {
	...
	requestID, found := trace.NormalizeID(state.Request.ID)
	if !found && state.Request.HTTPRequest != nil {
		requestID, found = trace.NormalizeID(state.Request.HTTPRequest.Header.Get(trace.RequestIDHeader))
	}
	if !found {
		generatedID, err := t.generator.Generate(state.Request.ConfigKey)
		if err != nil {
